...
const CampaignsPage: React.FC = () => {
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            case 'running': return 'bg-green-500/20 text-green-400 border border-green-500/30';
    ...
    return (
        <div className="p-6 font-sans text-gray-100">
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h1 className="text-2xl font-bold dark:text-white">Campaigns</h1>
                    <p className="text-gray-400 text-sm mt-1">Manage and monitor your outbound campaigns</p>
                </div>
                <button
                    onClick={() => setShowCreateModal(true)}
                    className="bg-emerald-600 hover:bg-emerald-700 text-white font-medium py-2 px-4 rounded-lg transition flex items-center shadow-lg shadow-emerald-900/20"
                >
            ...
            {loading ? (
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {campaigns.length === 0 ? (
                        <div className="col-span-full text-center p-12 dark:text-gray-400 bg-[#1E293B] rounded-lg border border-gray-700/50 dashed-border">
                            <div className="text-4xl mb-4 opacity-50">📱</div>
                            <h3 className="text-lg font-medium text-white mb-2">No campaigns found</h3>
                            <p className="text-gray-500 mb-6">Create your first campaign to start reaching out to contacts.</p>
                            <button
                                onClick={() => setShowCreateModal(true)}
                        ...
                        campaigns.map(campaign => (
                            <div
                                key={campaign.id}
                                className="group bg-[#1E293B] border border-gray-700/50 p-5 rounded-xl shadow-lg hover:shadow-xl hover:border-emerald-500/30 transition-all cursor-pointer relative overflow-hidden"
                                onClick={() => navigate(`/campaigns/${campaign.id}`)}
                            >
                                <div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <svg className="w-5 h-5 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg>
                                </div>
                                ...
                                <h3 className="text-lg font-bold text-white mb-1 group-hover:text-emerald-400 transition-colors">{campaign.name}</h3>
                                <p className="text-sm text-gray-400 mb-4 line-clamp-2 h-10">{campaign.description || 'No description provided.'}</p>

                                <div className="space-y-2 border-t border-gray-700/50 pt-4">
                                    <div className="flex items-center text-sm text-gray-300">
                                        <span className="w-5 text-center mr-2 text-gray-500">📞</span>
                                ...
                                <div className="flex gap-2 mt-4">
                                    {campaign.status === 'draft' && (
                                        <button
                                            ...
                                            className="flex-1 bg-gray-700 hover:bg-emerald-600 text-white py-2 rounded-lg text-sm font-medium transition"
                                        >
                                            Quick Start
                                        </button>
                                    )}
                                    <button
                                        ...
                                        className="px-4 py-2 bg-red-600/20 hover:bg-red-600 text-red-400 hover:text-white border border-red-600/30 rounded-lg text-sm font-medium transition"
            ...
            {showCreateModal && (
                <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
                    <div className="bg-[#1E293B] border border-gray-700 p-6 rounded-xl w-full max-w-lg shadow-2xl relative">
                        <button
                        ...
                        <form onSubmit={handleCreateCampaign}>
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input
                                        type="text"
                                        className="w-full bg-[#0F172A] border border-gray-700 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition"
                                ...
                                <div>
                                    ...
                                    <textarea
                                        className="w-full bg-[#0F172A] border border-gray-700 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition resize-none"
                                ...
                                <div>
                                    ...
                                    <select
                                        className="w-full bg-[#0F172A] border border-gray-700 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition appearance-none"
                                ...
                                <div>
                                    <div className="flex justify-between items-center mb-1.5">
                                        ...
                                        <label className="text-xs text-emerald-400 hover:text-emerald-300 cursor-pointer flex items-center">
                                    ...
                                    <textarea
                                        className="w-full bg-[#0F172A] border border-gray-700 rounded-lg px-4 py-2.5 text-white font-mono text-xs focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition"
                            ...
                            <div className="flex gap-3 mt-8">
                                ...
                                <button
                                    type="submit"
                                    className="flex-1 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-bold shadow-lg shadow-emerald-900/20 transition"
                                >
                                    Create Campaign
                                </button>
                            </div>
